      ...
      builder: (hCtrl) {
        return Scaffold(
          ...
          body: RefreshIndicator(
            onRefresh: () async{
             await hCtrl.getOrders(id: hCtrl.authUser.user.id);
                            ...
                            return GestureDetector(
                              onTap: () {
                                ...
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (context) => CustomerInvoice(order: hCtrl.orderList.orders![i]),
                                    ));
                              },
                              child: Padding(
                                padding: const EdgeInsets.all(8.0),
                                            ...
                                            child: Row(
                                              children: [
                                                ...
                                                Icon(
                                                  Icons.circle,
                                                  size: 15,
                                                  color: hCtrl.orderList.orders?[i].status == 0?Colors.green:Colors.red,
                                                ),
                                                SizedBox(
                                                  width: 10,
                                                ),
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            hCtrl.orderList.orders?[i].status == 2
                                                ? Text(
                                                    'Pending',
                                                    style: TextStyle(
                                                        color: Colors.red),
                                                  )
                                                : hCtrl.orderList.orders?[i]
                                                            .status ==
                                                        1
                                                    ? Text('PickedUp',
                                                        style: TextStyle(
                                                            color: Colors.blue))
                                                    :hCtrl.orderList.orders?[i].status==0 && hCtrl.orderList.orders?[i].accepted==false?
                                                    Text('Delivered',
                                                        style: TextStyle(
                                                            color: Colors.green)):
          
                                                    hCtrl.orderList.orders?[i].accepted==true && hCtrl.orderList.orders?[i].status==0
                                                  ? Icon (
                                                        Icons.done_outlined,
                                                        color: Colors.green,
                                                      ):SizedBox()
